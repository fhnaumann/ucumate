 ...
 */
public class VersionSpecificUCUMRegistry implements IUCUMRegistry {

    private static final Logger log = LoggerFactory.getLogger(VersionSpecificUCUMRegistry.class);

    private static final ValidatorService validatorService = new Validator();

    private final UcumVersion version;
    ...
    private UCUMDefinition.UCUMEssence loadUCUMEssence(InputStream ucumEssenceXML) {
        try {
            ...
            return LogUtil.logAndThrow(log, e, "Failed to load ucum-essence file for version {}", version);
        }
    }
}
