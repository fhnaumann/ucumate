package org.example.graham.functionaltests;
...
public class UCUMFunctionalTests {
    ...
    public void testXmlElement(String testName, FunctionalTestType testType, Element testCase) throws IOException {
        if (testType.equals(FunctionalTestType.HISTORY))
            ...
            throw new IllegalStateException("unknown test type "+ testCase.getNodeName());
    }
    ...
    private void runMultiplicationCase(Element x) {
        ...
        Expression.Term term1 = ((Validator.Success) Validator.validate(u1)).term();
        Expression.Term term2 = ((Validator.Success) Validator.validate(u2)).term();
    ...
    private void runConversionCase(Element x) {
        ...
        Expression.Term fromTerm = ((Validator.Success) Validator.validate(srcUnit)).term();
        Expression.Term toTerm = ((Validator.Success) Validator.validate(dstUnit)).term();
